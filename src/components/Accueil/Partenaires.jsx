...
const Partenaires = () => {
  ...
  const prevSlide = useCallback(() => {
    setCurrentIndex((prev) => (prev - 1 + partenaires.length) % partenaires.length);
  }, [partenaires.length]);
  ...
  const getVisibleLogos = () => {
    ...
    for (let i = -halfVisible; i <= halfVisible; i++) {
      const index = (currentIndex + i + partenaires.length) % partenaires.length;
      logos.push({
        ...partenaires[index],
      ...
      <div className="partenaires-container">
          ...
          <div className="partenaires-track">
            {getVisibleLogos().map((partenaire) => (
              <div
                ...
                className={`partenaires-item ${partenaire.position === 0 ? 'partenaires-center' : ''}`}
                style={{
                  transform: `translateX(${partenaire.position * 100}%) scale(${partenaire.position === 0 ? 1.2 : 0.85})`,
                  opacity: Math.abs(partenaire.position) <= 1 ? 1 : 0.5,
                  zIndex: partenaire.position === 0 ? 10 : 5 - Math.abs(partenaire.position)
                }}
              >
        ...
        <div className="partenaires-dots">
          {partenaires.map((_, index) => (
            <button
              key={index}
              className={`partenaires-dot ${index === currentIndex ? 'active' : ''}`}
